...
ll f(vector<ll>&a, vector<ll>&b, ll x, ll i, vector<vector<ll>>&dp) {
    if (i < 0 )return 0;
    if (dp[i][x] != -1)return dp[i][x];
    if (x - a[i] >= 0) {
        return dp[i][x] = max(f(a, b, x - a[i], i - 1, dp) + b[i], f(a, b, x, i - 1, dp));
    } else {
        return dp[i][x] = f(a, b, x, i - 1, dp);
    }
...
void solve() {
    ...
    for (auto &x : a)cin >> x;
    for (auto &x : b)cin >> x;
    vector<ll>cur(x + 2), prev(x + 2);
    int cnt = 0;
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j <= x; j++) {
            cur[j] = prev[j];
            if (j - a[i - 1] >= 0) {
                cur[j] = max(cur[j], prev[j - a[i - 1]] + b[i - 1]);
            }
        }
        prev = cur;
        cur.assign(x + 2, 0);
