...
long long bs_to_decimal(const string& binaryString) {
	return bitset<32>(binaryString).to_ulong();
}
 
string binary_string(long long number) {
...
ll mod = 1e9 + 7;
ll f(int x, ll sum,  vector<vector<ll>>& dp) {
	if (x == 0 || sum < 0)return 0;
	if (sum == 0)return 1;
	if (dp[sum][x] != -1) return dp[sum][x] % mod;
	return dp[sum][x] = (f(x - 1, sum - x, dp) + f(x - 1, sum, dp)) % mod;
}
